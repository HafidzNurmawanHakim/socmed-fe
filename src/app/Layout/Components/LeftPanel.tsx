	...
	return (
		<div
			...
			className={`bg-white hidden xl:block ${
				openPanel ? "basis-1/5 lg:basis-1/4  xl:basis-2/8 2xl:basis-3/8" : "basis-24"
		...
		>
			<div className="h-16 pt-2 mb-2">
				<button className={`btn btn-ghost ${openPanel ? "ml-4" : "ml-1"}`}>
					...
					<span className={`text-normal  ${openPanel ? "visible" : "invisible"}`}>
						Coffee
					</span>
				</button>
			</div>
					...
					return (
						<li key={i + item.title} className="custom-transition">
							...
							>
								{item.icon}
								<span
									className={`custom-transition text-teal ml-4 ${openPanel ? "" : "hidden"}`}
								>
									{item.title}
								</span>
							</a>
